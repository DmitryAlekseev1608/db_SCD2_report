class Account:
    ...
    def insert_date_in_table(self, cursor_db, conn_db):
        ...
        cursor_db.execute(""" delete from public.alex_STG_account;
                            delete from public.alex_STG_account_del;
                        """)
        conn_db.commit()

        # 2. Захват данных из источника (измененных с момента последней загрузки) в стейджинг
        cursor_db.execute(f""" insert into public.alex_STG_account(
                          account_num,
                          valid_to,
                          client,
                          update_dt )
        ...
        cursor_db.execute(f"""insert into public.alex_DWH_DIM_account_HIST(
                          ...
                          select
                                ...
                                false
                            from public.alex_STG_account stg
                            left join public.alex_DWH_DIM_account_HIST tgt
    ...
    # 5.1. Обновим имеющуюся в target строчку и закроем ее по дате.
        cursor_db.execute(f"""update public.alex_DWH_DIM_account_HIST
                        set
                        ...
                        from (
                            select 
                                ...
                                null 
                            from public.alex_STG_account stg
                            inner join public.alex_DWH_DIM_account_HIST tgt
                            on stg.account_num = tgt.account_num
                        ...
                        where alex_DWH_DIM_account_HIST.account_num = tmp.account_num
                        and alex_DWH_DIM_account_HIST.effective_to = TO_DATE('2999-01-01', 'YYYY-MM-DD')
                        """)
        conn_db.commit()      
    ...
    # 6. Обновление той строчки, что удалили в источнике (формат SCD2)
        cursor_db.execute(f"""update public.alex_DWH_DIM_account_HIST
                        ...
                        where account_num in (
                            ...
                            from public.alex_DWH_DIM_account_HIST tgt
                            left join public.alex_STG_account_del stg
                            on stg.account_num = tgt.account_num
    ...
    # 7. Обновление метаданных
        cursor_db.execute("""update public.alex_META_meta
                        set max_update_dt = coalesce( (select max( update_dt ) from public.alex_STG_account ), ( select max_update_dt
                            from public.alex_META_meta where schema_name='public' and table_name='public.alex_DWH_DIM_account_HIST' ) )
                        where schema_name='public' and table_name = 'public.alex_DWH_DIM_account_HIST';
                        """)
        conn_db.commit()